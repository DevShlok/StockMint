    ...
    if request.method == 'POST':
        ...
        try:
            df = yf.download(
                ...
                auto_adjust=True,  # Explicitly set
                multi_level_index=False  # Add this parameter if needed
            )
            if df.empty:
        ...
        if asset_type == 'stock':
            ...
            if len(X_test) == 0:
                context['error'] = "Not enough data for prediction."
                return render_template('index.html', **context)
            y_pred = stock_model.predict(X_test, verbose=0) * (1/scaler.scale_)
            y_true = scaled_test[100:] * (1/scaler.scale_)
...
if __name__ == '__main__':
    ...
    app.run(debug=True, use_reloader=False)  # Disable reloader to fix WinError 10038
